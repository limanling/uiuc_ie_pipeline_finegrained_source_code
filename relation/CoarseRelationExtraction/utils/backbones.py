import os
import sys
CURRENT_DIR = os.path.dirname(os.path.realpath(__file__)) #os.path.abspath(os.path.dirname(__file__))
...
from feature_extraction import padding_list
import pickle
import numpy as np
import torch
...
from generate_CS import rel_map_ERE, ERE_to_AIDA_final_ouput, generate_cs_file
from aida2inter import assembly_list, doc_to_men_to_type, doc_to_segment, doc_to_offset
    ...
    with open(intermediate_path, "w", encoding="utf-8") as fmodel:
        for doc_id in docseg2mention:
            for segment in docseg2mention[doc_id]:
                list_offset = []
                # print('len(docseg2mention[doc_id][segment])', len(docseg2mention[doc_id][segment]))
                # print(Counter([str(elem) for elem in docseg2mention[doc_id][segment]]))

                for i in range(len(docseg2mention[doc_id][segment])):
                    mention_i = docseg2mention[doc_id][segment][i]
                    if mention_i in doc2segoffset[doc_id][segment]:
                        # single-word mention
                        mention_loffset = doc2segoffset[doc_id][segment].index(mention_i)
                        list_offset.append([mention_loffset, mention_loffset])
                    else:
                        # multiple-word mention
                        temp = []
                        for j in range(len(doc2segoffset[doc_id][segment])):
                            if mention_i[0] in doc2segoffset[doc_id][segment][j]:
                                temp.append(j)
                            if mention_i[1] in doc2segoffset[doc_id][segment][j]:
                                temp.append(j)
                    ...
                    for item in out_list:
                        if len(doc2segtext[doc_id][segment]) < max_len:
                            ...
                            if item[0][1] != item[1][0]:
                                ...
                                outstr = "1" + " " + str(item[0][0]) + " " + str(item[0][1]) + " " + \
                                         str(item[1][0]) + " " + str(item[1][1]) + " " + \
                                         output_sentence + "\t" + mention2type[ltf_mention1] + " " + \
                                         mention2type[ltf_mention2] + "\t" + ltf_mention1 + " " + \
                                         ltf_mention2 + "\t" + output_seg + "\n"

                                if outstr not in seen_outs:
                                    fmodel.write(outstr)
                                    seen_outs.add(outstr)
