...
class AIDAReader(object):
    ...
    def getNext(self, normalize_digits=True, symbolic_root=False, symbolic_end=False):
        line = self.__source_file.readline().strip().decode("utf-8")
        if not line:
            return None

        # print('line', line)
        seg_id, temp = line.split("\t")
        tokens = [tuple(tok_tag.rsplit("||")) for tok_tag in temp.split()]
        # print(tokens)
        ...
        for tok, pos in tokens:
            ...
            word = utils.DIGIT_RE.sub(b"0", tok) if normalize_digits else tok
            head = 0
            type = "obj"
        ...
        return DependencyInstance(
            Sentence(words, word_ids, char_seqs, char_id_seqs, segment_id=seg_id),
            postags,
            pos_ids,
            heads,
            types,
