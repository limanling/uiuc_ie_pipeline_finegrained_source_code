...
def get_hidden_states(hiddens, precision=None, output_all=False):
    lines = []
    for t_b, s_b, h_b, l_b in hiddens:
        for t_s, s_s, h_s, l_s in zip(t_b, s_b, h_b, l_b):
            h_s = h_s[:len(t_s)]
            for t, s, h, l in zip(t_s, s_s, h_s, l_s):
                if l != 'O' or output_all:
                    if type(precision) is int:
                        h = ','.join(['{i:.{p}f}'.format(i=i, p=precision)
                                      for i in h])
                    else:
                        h = ','.join([str(i) for i in h])
                    lines.append('{}\t{}\t{}'.format(t, s, h))
    return lines