...
def run_tagger(model, vocabs, bio_str, to_bio=True, separator=' ', conf=True,
               elmo=True, return_hidden=True):
    ...
    for batch in DataLoader(test_set, batch_size=20, shuffle=False,
        ...
        if return_hidden:
            hidden_states.extend(
                get_hidden_states([(tokens, spans, lstm_out, preds)],
                                  precision=8))
    hidden_states = '\n'.join(hidden_states)
    return convert_result(results, to_bio, separator=separator, conf=conf).strip(), hidden_states
...
def tagging():
    try:
        ...
        if lang_code == 'en':
            # NAM
            eng_nam_bio_str, eng_nam_hid_str = run_tagger(eng_nam_model, eng_nam_vocabs, bio_str)
            eng_nam_bio_str = eng_nam_post_process(eng_nam_bio_str)
            eng_nam_tab_str = convert_bio2tab(eng_nam_bio_str, conf_col=2)
            # NOM
            eng_nom_5type_bio_str, eng_nom_5type_hid_str = run_tagger(eng_nom_5type_model, eng_nom_5type_vocabs, bio_str)
            eng_nom_wv_bio_str, eng_nom_wv_hid_str = run_tagger(eng_nom_wv_model, eng_nom_wv_vocabs, bio_str)
            ...
            # PRO
            eng_pro_bio_str, eng_pro_hid_str = run_tagger(eng_pro_model, eng_pro_vocabs, bio_str)
            ...
            result = {
                ...
                'tsv': eng_fet_tsv_str,
                'en_nam_hid': eng_nam_hid_str,
                'en_nom_5type_hid': eng_nom_5type_hid_str,
                'en_nom_wv_hid': eng_nom_wv_hid_str,
                'en_pro_hid': eng_pro_hid_str,
            }
            return jsonify(result)
        elif lang_code == 'ru':
            # NAM
            rus_nam_5type_bio_str, rus_nam_5type_hid_str = run_tagger(rus_nam_5type_model, rus_nam_5type_vocabs,
                                        bio_str, elmo=False)
            rus_nam_wv_bio_str, rus_nam_wv_hid_str = run_tagger(rus_nam_wv_model, rus_nam_wv_vocabs, bio_str,
                                     elmo=False)
            ...
            result = {
                ...
                'tsv': rus_fet_tsv_str,
                'ru_nam_5type_hid': rus_nam_5type_hid_str,
                'ru_nam_wv_hid': rus_nam_wv_hid_str,
            }
            return jsonify(result)
        elif lang_code == 'uk':
            # NAM
            ukr_nam_5type_bio_str, ukr_nam_5type_hid_str = run_tagger(ukr_nam_5type_model, ukr_nam_5type_vocabs,
                                        bio_str, elmo=False)
            ukr_nam_wv_bio_str, ukr_nam_wv_hid_str = run_tagger(ukr_nam_wv_model, ukr_nam_wv_vocabs,
                                     bio_str, elmo=False)
            ...
            result = {
                ...
                'tsv': ukr_fet_tsv_str,
                'uk_nam_5type_hid': ukr_nam_5type_hid_str,
                'uk_nam_wv_hid': ukr_nam_wv_hid_str
            }
            return jsonify(result)
        else:
...
@app.route('/typingbio', methods=['POST'])
def typingbio():
    try:
