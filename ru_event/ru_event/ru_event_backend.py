...
def ru_event(ltf_folder_path, input_edl_bio_file_path, input_rsd_folder_path, entity_cs_file_path, out_folder_path, model_file_path, \
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--ltf_folder_path", #default=os.path.join(cwd, 'data/source/ru_all'),
        help="input folder path containing LTF files"
    )
    
    parser.add_argument(
        "--input_edl_bio_file_path", #default=os.path.join(cwd, 'data/source/edl_results/ru.nam+nom.tagged.bio'),
        help="the BIO formated entity extraction results from Boliang's system"
    )
    
    parser.add_argument(
        "--input_rsd_folder_path", #default=os.path.join(cwd, 'data/source/ru_all_rsd/'),
        help="the path to the folder containing rsd files"
    )

    parser.add_argument(
        "--entity_cs_file_path", #default=os.path.join(cwd, 'data/source/edl_results/ru.merged.cs'),
    ...
    out_folder_path = 'aida_event_ru/intermediate_files'
    # if not os.path.exists(out_folder_path):
    #     os.makedirs(out_folder_path, exist_ok=True)
    result_folder_path = args.output_dir  #'aida_event_ru/results'
    if not os.path.exists(result_folder_path):
        os.makedirs(result_folder_path, exist_ok=True)

    model_file_path = os.path.join(cwd, 'trigger_code/dnn_pytorch/best_model.pth.tar')
    input_file_path = os.path.join(out_folder_path, 'ru_all.bio')
    output_file_path_1 = os.path.join(out_folder_path, 'ru_all_triggers_offset.bio')
    batch_size = 50
    gpu = 0

    input_trigger_offset_file_path = os.path.join(out_folder_path, 'ru_all_triggers_offset.bio')
    output_file_path_2 = os.path.join(out_folder_path, 'ru_all_merged.bio')

    input_merged_bio_file_path = os.path.join(out_folder_path, 'ru_all_merged.bio')

    model_folder_path = os.path.join(cwd, 'cnn_code/runs/ru_1535939310/checkpoints/')
    out_file_path = os.path.join(out_folder_path, 'ru_all.tsv')

    cs_file_path = args.output_event_cs  #os.path.join(result_folder_path, 'ru_all.cs')
    trig_offset_file_path = os.path.join(out_folder_path, 'ru_all_triggers_offset.bio')
    cnn_out_file_path = os.path.join(out_folder_path, 'ru_all.tsv')


    ru_event(ltf_folder_path, input_edl_bio_file_path, input_rsd_folder_path, entity_cs_file_path, out_folder_path, model_file_path, \
        input_file_path, output_file_path_1, batch_size, gpu, input_trigger_offset_file_path, output_file_path_2, input_merged_bio_file_path, \
        model_folder_path, out_file_path, cs_file_path, trig_offset_file_path, cnn_out_file_path)
