...
def prepare_dataset(sentences, feat_column,
    ...
    return data
#ann:
def prepare_dataset_(sentences, word_to_id, tag_to_id, lower=False, is_train=True):
    """
    Prepare the dataset. Return a list of lists of dictionaries containing:
        - word indexes
        - word char indexes
    ...
    for i, s in enumerate(sentences):
        data.append(
            prepare_sentence_(s, word_to_id, tag_to_id, lower, is_train=is_train)
        )

    return data

#ann:
def prepare_sentence_(sentence, word_to_id, tag_to_id, lower=False, is_train=True):
    """
    Prepare a sentence for evaluation.
    """
    def f(x): return x.lower() if lower else x
